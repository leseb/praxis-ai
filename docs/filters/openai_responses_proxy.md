...
Requests classified with `background: true` are rejected unless the load balancer selected a cluster whose `application_provider` is `openai` or whose endpoint host is exactly `api.openai.com`. A configured local response store always causes rejection because local polling cannot observe OpenAI's asynchronous state transitions. Because cluster selection happens during the request-header phase, a pipeline that permits OpenAI background mode must order this filter after its router and load balancer. Missing or malformed selected-upstream identity fails closed.
