...
/// (in request order, including servers that resolve to zero tools) is surfaced to
/// the client: buffered responses list it in `output`, and streaming responses
/// synthesize its `output_item.added` → `mcp_list_tools.in_progress` →
/// `mcp_list_tools.completed` → `output_item.done` lifecycle ahead of the model
/// output. A previous-response cache hit surfaces the same item without re-calling
/// `tools/list`, and internal retries reuse the item and its id rather than
/// emitting a second discovery.
///
/// # YAML
///
/// ```yaml
