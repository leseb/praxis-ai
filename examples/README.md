...
| [file-search-dispatch.yaml](configs/openai/responses/file-search-dispatch.yaml) | Demonstrates hosted `file_search` execution under the unified agentic loop (#1046). `openai_agentic_loop` is the sole loop owner: it parses each model response, records the `file_search_call` items it sees as assignments, and publishes the single continuation signal (`action=loop|done`). `openai_file_search_dispatch` is a pure request-phase dispatcher: at request- body EOS on each IRR re-entry it executes the assigned calls against the vector store and reconciles each item in place, then the owner prepares the next inference request |
...
| [full-flow-agentic.yaml](configs/openai/responses/full-flow-agentic.yaml) | Runs the complete Responses API pipeline through an agentic iterative_request_router that executes hosted file_search, web_search_dispatch, and MCP tool calls in a model-tool-model loop, persisting both buffered and streaming (`stream: true`) responses |
...
| [web-search-chat-completions-fixture.yaml](configs/openai/responses/web-search-chat-completions-fixture.yaml) | Single-upstream fixture configuration for recording the private Chat Completions function representation of a Responses web_search_dispatch tool |
