...
| `user_credential` | string | no | Optional callout-credential slot. When configured, Files API `file_id` requests use that caller-scoped value as their `Authorization` header. Client-controlled `file_url` fetches never use this credential. |
...
| `timeout_ms` | integer | no | HTTP timeout in milliseconds for Files API callout requests. Inside an iterative request router, the effective timeout is capped by the router's remaining deadline. |
