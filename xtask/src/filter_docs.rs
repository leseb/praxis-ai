...
fn parse_shared_config_items(root: &Path) -> ModuleItems {
    ...
    let dirs = if praxis_root.is_dir() {
        // The praxis workspace nests its crates under `crates/`; older layouts
        // kept them at the root. List both candidate paths — `collect_rs_files`
        // silently skips whichever directory is absent.
        vec![
            praxis_root.join("crates/filter/src/builtins/http/payload_processing"),
